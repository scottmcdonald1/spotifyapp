...
export default function Artists({user, data, albumData, id}) {
  ...
  const albumItems = albumData.items.map(item => {
    ...
    return (
      <>
      <div className="grid grid-cols-4 bg-white border border-ombreNaturelle/50 rounded px-2 py-2 shadow-spread items-end">
  ...
  const test =  id ? (
    <div className="w-full min-h-screen grid gap-2 grid-cols-1 justify-center items-center py-20">
      ...
      <div className="w-full grid grid-cols-3 border border-ombreNaturelle31/60 shadow-sharp rounded px-2 py-4">
        ...
        <div className="col-span-2 grid grid-cols-1 grid-rows-3 bg-white border border-ombreNaturelle31/60 shadow-spread rounded px-2 py-4">
      ...
      <div className="w-full p-8 break-words grid gap-4 grid-cols-1">
